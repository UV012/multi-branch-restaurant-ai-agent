# Multi-Branch Restaurant AI Agent
...
**Stack:** FastAPI · LangGraph · PostgreSQL · React · Ollama (DeepSeek R1)
...
- **PostgreSQL does the safety-critical work, not the LLM.** An LLM can be persuaded, confused, or just wrong. Two guarantees in this system are enforced at the database level, so they hold even if the agent's reasoning goes off the rails:
...
- Menu and per-branch stock editor
- Branch and table configuration
...
├── models/         # SQLAlchemy models (branches, menu, orders, reservations, users)
├── routes/         # FastAPI routers (customer + staff APIs)
